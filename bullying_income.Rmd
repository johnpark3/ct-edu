...
```
```{r}
svyby(~hhincome, ~region, income_des, svyquantile, quantiles = c(0.2, 0.5, 0.95), keep.var = F) %>%
  as.data.frame() %>%
  as_tibble() %>%
  setNames(c("region", "q20", "q50", "q95")) %>%
  mutate_if(is.numeric, round) %>%
  mutate(ratio95_20 = round(q95 / q20, digits = 3))
```
In `New Haven Town`, the highest-earning 5% makes 13.3x more money than the bottom 20%.
In `West Haven, Milford & Orange Towns`, the highest-earning 5% makes 8x more money than the bottom 20%.
In `East Haven, Branford, Guilford, Madison & North Branford Towns` and `Hamden, Ansonia, Seymour, Derby, Woodbridge & Bethany Towns`, the highest-earning 5% makes 8.4x more money than the bottom 20%.
